...
// wd, ht: button dims
int timPaintDelay = 0;
int tmp = 0, wd = 0, ht = 0, capCallFrmResize = 0, scrShtOrBmpLoad;
int xCurrentScroll, yCurrentScroll; // Updated in ScrollInfo
...
void SizeControls(BITMAP bmp, HWND hWnd, int& updatedxCurrentScroll, int& updatedyCurrentScroll, int resizeType = -1, int curFmWd = 0, int curFmHt = 0, BOOL newPic = FALSE);
...
LRESULT CALLBACK MyBitmapWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZING:
    {
        ...
        if (timPaintDelay)
        {
            ...
            // which then reduces WM_PAINT calls. This must be turned off in WM_PAINT
            // or there will be automatic window invalidations with COLOR_WINDOW + 1
            // after each bitblt in WM_PAINT, which causes much flicker. Better handling of the timer
    ...
    case WM_TIMER:
    {
        ...
        case IDT_DRAGWINDOW:
        {
            ...
            if (tmp)
            {
                ...
                if (!AdjustImage(hWnd, hBitmap, bmp, gps, hdcMem, hdcMemIn, hdcScreen, hdcScreenCompat, hdcWinCl, bmpWidth, bmpHeight, xNewSize, yNewSize, updatedxCurrentScroll, updatedyCurrentScroll, ((stretchChk) ? 2 : ((scrShtOrBmpLoad == 2) ? 1 : 0))))
    ...
    case WM_COMMAND: // WM_PARENTNOTIFY if hWndGroupBox is parent to controls (not the case here)
    {
       ...
       //  HWND hwndButton; // Button at position of cursor
        ...
        case IDC_CHK:
        {
            ...
            if (stretchChk)
            {
                if (scrollStat = ScrollInfo(hWnd, 0, 0, 0, xNewSize, yNewSize, xNewSize - wd, yNewSize - ht))
                ...
                if (gps == Ok)
                {
                    ...
                    if (hBitmap)
                    {
                        ...
                        if (xCurrentScroll || yCurrentScroll)
                        {
                            ...
                            SizeControls(bmp, hWnd, xCurrentScroll, yCurrentScroll, ((isMaximized) ? SIZE_MAXIMIZED : SIZE_RESTORED), xNewSize, yNewSize, TRUE);
                        }
...
void SizeControls(BITMAP bmp, HWND hWnd, int& updatedxCurrentScroll, int& updatedyCurrentScroll, int resizeType, int curFmWd, int curFmHt, BOOL newPic)
{
